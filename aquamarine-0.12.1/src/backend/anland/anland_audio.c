...
/* PipeWire owns the duplicated fd when the source is created with close=true.
 * Destroying it removes the fd from epoll and closes the duplicate, so a
 * producer-side close cannot leave a permanent HUP/ERR dispatch behind. */
...
void anland_audio_set_fd(int audio_fd)
{
    ...
    /* Detach the old owned source before installing a new socket. */
    detach_audio_fd_locked(a);

    if (audio_fd >= 0) {
        /* Match KWin: keep an owned duplicate. The producer may close its
         * original during fallback; PipeWire then owns and closes this copy
         * when the source is destroyed. */
        int owned_fd = fcntl(audio_fd, F_DUPFD_CLOEXEC, 3);
        if (owned_fd >= 0) {
            a->io = pw_loop_add_io(loop, owned_fd, SPA_IO_IN, true, on_audio_readable, a);
            if (a->io)
                a->audio_fd = owned_fd;
            else
                close(owned_fd);
