...
/* Receive the input-extend-fds message that follows an INPUT_TYPE_RESOURCE event.
 * Returns the number of fds received, or -1 on error. */
int  poll_input_event_extend_fds(display_ctx *ctx, int *fds, int fd_count, int timeout_ms);
/* Convenience wrapper: calls poll_input_event_extend_fds for service fds. */
