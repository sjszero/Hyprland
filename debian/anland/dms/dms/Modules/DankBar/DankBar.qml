...
Item {
    ...
    Variants {
        ...
        model: {
            ...
            const orientationMode = root.barConfig?.orientationMode || "all";
            let screens;

            if (prefs.includes("all") || (typeof prefs[0] === "string" && prefs[0] === "all")) {
                screens = Quickshell.screens;
            } else {
                screens = Quickshell.screens.filter(screen => SettingsData.isScreenInPreferences(screen, prefs));
                if (screens.length === 0 && root.barConfig?.showOnLastDisplay && Quickshell.screens.length === 1)
                    screens = Quickshell.screens;
            }

            // Anland changes the single output geometry when Android rotates.  Keep
            // the phone-specific two-bar layout in portrait, but restore the stock
            // DMS bar in landscape.
