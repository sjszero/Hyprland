...
Singleton {
    ...
    // Portrait detection: true when the primary screen is taller than wide.
    // Used to adapt DMS modules for phone-style portrait displays.
    readonly property bool isPortrait: {
        const screens = Quickshell.screens;
        if (!screens || screens.size === 0)
            return false;
        const s = Array.from(screens.values())[0];
        return s.height > s.width;
    }

    // Convenience: logical screen width and height (first screen)
    readonly property real screenWidth: {
        const screens = Quickshell.screens;
        if (!screens || screens.size === 0) return 0;
        return Array.from(screens.values())[0].width;
    }
    readonly property real screenHeight: {
        const screens = Quickshell.screens;
        if (!screens || screens.size === 0) return 0;
        return Array.from(screens.values())[0].height;
    }
